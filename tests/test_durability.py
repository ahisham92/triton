...
def test_bs6349_tables_not_loaded_keep_the_values():
    d = defaults("bs6349", "bs6349", 50)
    assert d["covers"] is None and d["corrosion"] is None and len(d["notes"]) == 2
...
def test_elements_take_the_project_values_unless_set():
    ...
    assert with_project_grades(SheetPileInput(), m, d).corrosion_loss_per_face == 1.75
