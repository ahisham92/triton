...
def test_cage_export_endpoint(tmp_path, monkeypatch):
    ...
    pid = client.post(
        "/api/projects", json={"info": {"name": "Berth 1"}, "element_names": ["Pile(1)"]}
    ).json()["id"]
    assert client.get(f"/api/projects/{pid}/design/piles/cages.json").status_code == 404
    data = xlsx_bytes({"Pile(1)-PT-B-Apron": pile_sheet(), "Pile(1)-QP": pile_sheet()})
    client.post(f"/api/projects/{pid}/workbook", files={"file": ("s.xlsx", data)})
    client.post(f"/api/projects/{pid}/design/piles")
    r = client.get(f"/api/projects/{pid}/design/piles/cages.json")
    assert r.status_code == 200
    assert "Berth_1-pile-cages.json" in r.headers["content-disposition"]
    assert r.json()["piles"][0]["element"] == "Pile(1)"
