...
from triton.project import LoadFactor, Project
...
def test_factors_apply_to_chosen_sheets_only():
    project = Project(load_factors=[LoadFactor(factor=1.35, sheets=["Pile(1)-PT-B-Apron"])])
    sheets = factored_elements(project, workbook())["Pile(1)"]
...
def test_design_reports_the_multiplier():
    project = Project(load_factors=[LoadFactor(factor=1.35, sheets=["Pile(1)-PT-B-Apron", "Nope-QP"])])
    project.add_elements(["Pile(1)"])
    project.elements["Pile(1)"].head_level = 1.0
    out = run_piles(project, workbook())
    (pile,) = out["piles"]
...
def test_a_sheet_is_multiplied_once():
    with pytest.raises(ValidationError, match="more than one"):
        Project(load_factors=[LoadFactor(sheets=["A-QP"]), LoadFactor(factor=1.5, sheets=["A-QP"])])
